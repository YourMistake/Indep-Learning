...
void Stecker::removeWiring(char a)
{
	char b = m_map[a-'A'];
	m_map[a-'A'] = a - 'A';
	m_map[b] = b;
...
void Scrambler::setup()
{
	...
	m_rotor[0]->ringSetting(1);
	m_rotor[1]->ringSetting(1);
	m_rotor[2]->ringSetting(1);
	m_rotor[3]->ringSetting(1);
	m_rotor[4]->ringSetting(1);
...
void Scrambler::setupBP()
{
	m_isBP = true;

	// Bombe wiring??
	m_rotor[0]->loadWiring("KFLNGMHERWAOUPXZIYVTQBJCSD");
	m_rotor[1]->loadWiring("FBKELTJSVYCMIXUNDRHAOQZGWP");
	m_rotor[2]->loadWiring("PCEGIKMDQSUYWAOZFJXHBLNVTR");
	m_rotor[3]->loadWiring("YDGUQXRBLCASWKTJZNPHVIMFEO");
	m_rotor[4]->loadWiring("HFNYCEUJLWBXSVGQKOADZPMTRI");
	...
	m_rotor[0]->ringSetting(26);
	m_rotor[1]->ringSetting(26);
	m_rotor[2]->ringSetting(26);
	m_rotor[3]->ringSetting(26);
	m_rotor[4]->ringSetting(26);
...
const char* Scrambler::getIndicator() const
{
	...
	const_cast<Scrambler*>(this)->m_indicator[ROTORSLOTS] = '\0';
...
void Enigma::removeSteckerWire(char a)
{
	m_stecker.removeWiring(a);
}
