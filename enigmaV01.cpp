...
/*    _________________________________________________________  *\
     / ------------------------------------------------------- \
    / /  ____    _____    _   ________   _______        ____  \ \
   / /  /  __|  / ___ \  | | /  ____  |  |      \      / _  \  \ \
  / /  /  /    | |   \ | | | | |    \_|  | |\ \  \    / / | |   \ \
 / /  |  /     | |   | | | | | |         | | | \  \  / / /  |    \ \
/ /   | |___   | |   | | | | | |  _____  | | | || | / / /   |     \ \
...
\ \   | |      | |   | | | | | |     | | | | | || | |   / | |     / /
 \ \  |  \     | |   | | | | | |     | | | | | || | |  /  | |    / /
  \ \  \  \__  | |   | | | | | |_____| | | | | || | | |   | |   / /
   \ \  \____| |_|   |_| |_| \_________/ |_| |_||_| |_|   |_|  / /
    \ \                                                       / /
     \ ------------------------------------------------------/ /
...
//***************************************************************************************************************************************\\
        ...
        keyword_array[number_of_keywords] = {"DERFUHRER", "WETTERBERICHT"};
      //***************************** End Bombe Variables ****************************\\      
...
//***************************************************************************************************************************************\\   
    ...
    //To Assign a Character twice.
    //char plug_board[26] = {'Z','P','H','N','M','S','W','C','I','Y','T','Q','E','D','O','B','L','R','F','K','U','V','G','X','J','A'};
	char plug_board[26]         = {'*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*','*'};
...
string sanatize(string string_in){
    string input =  string_in;
    string sanatized_input = "";
...
string enigma(string string_in){
    //string to work with
    string sanatized_input = sanatize(string_in);
    ...
    //**************************Find the Equivalent on the Plugboard Down**********************************\\| 
        character = plug_board[base_location]; 
    ...
    //*****************************************************************************************************\\|       
        ...
        character = plug_board[plug_board_location];    
                    
        for(int base_iterator = 0; base_iterator < alphabet_normal_length; base_iterator++){
            if(character == plug_board[base_iterator]){
                base_location = base_iterator;
                break;
            } 
        }
...
string format_to_enigma_standard(string input_string){
    string return_string = "";
    for(int i = 0; i < output.length(); i++){
        if(i % 5 == 0 && i != 0){
            return_string = return_string + " ";
...
string enigma_getline(string string_in){
...
*/
string Bombe(string sanatized_input){
...
void fill_plugboard(){      
    ...
    for(int i = 0; i <= plug_board.length(); i++){
...
int main(){
    ...
    cout << "      / ------------------------------------------------------- \\" << endl; 
    cout << "     / /  ____    _____    _   ________   _______        ____  \\ \\" << endl; 
    ...
    cout << "  \\ \\  |  \\     | |   | | | | | |     | | | | | || | |  /  | |    / /" << endl; 
    cout << "   \\ \\  \\  \\__  | |   | | | | | |_____| | | | | || | | |   | |   / /" << endl; 
    cout << "    \\ \\  \\____| |_|   |_| |_| \\_________/ |_| |_||_| |_|   |_|  / /" << endl; 
    cout << "     \\ \\                                                       / /" << endl; 
    cout << "      \\ ------------------------------------------------------/ /" << endl; 
    cout << "       \\_______________________________________________________/" << endl; 
    
    //list options
    cout << endl << "[1] Verschlusselung oder Entschlusselung(Encrypt/Decrypt from input)"
    << endl << "[2] Verschlusselung von text(Encrypt/Decrypt From a File)"
    << endl << "[3] Bombe, Crack an Enigma Message From Text" 
    << endl << "[4] Bombe, Crack an Enigma Message From a File" 
    << endl << "[5] Einstellungen(Settings)"
    << endl << "[6] Ausgang" << endl; 
	do {
	    ...
	    switch (option){ //Detects the option
	        case 1:{
                ...
                cout << "Enter a string without punctuation, numbers or special characters: \n";
	            cin >> input;
                string sanatized_input = "";
                sanatized_input = sanatize(input);
                cout << "The Message reads: " << enigma(sanatized_input) << endl;
	            break;
	        }
	        ...
	        case 2:{     
                ...
                cout << "Wie heisst der Nam des Textes mit .txt: \n";
	            cin >> input_file_name;
                //output .txt file
                cout << "Wo ist es Gehen mit .txt?: \n";
                ...
                if(inputFile.is_open()){
                    ...
                    while(getline(inputFile, input)){
                        sanatized_input = sanatize(input);
                        string out_string = enigma(sanatized_input);
                        cout << "The message Reads: " << out_string << endl;
                ...
                //if the file cannot be opened, tell me so!
                    ...
                    cout << "Keiner Text heisst " << inputFile << endl;
                    break;
                }
            ...
            case 3:{   
                ...
                cout << "Enter the encrypted Enigma Message without punctuation, numbers or special characters: \n";
	            cin >> input;
                string sanatized_input = "";
                sanatized_input = sanatize(input);
                cout << "The Chosen Message from Bombe Reads: " << bombe(sanatized_input) << endl;
            ...
            case 4:{   
                ...
                string output_file_name = "";
                //take the user given .txt file name
                cout << "What is the File Name With .txt extentsion: \n";
	            cin >> input_file_name;
                //output .txt file
                cout << "Where is the Decryption going with .txt?: \n";
                cin >> output_file_name;
                ...
                if(inputFile.is_open()){
                    ...
                    while(getline(inputFile, input)){
                        sanatized_input = sanatize(input);
                        string out_string = bombe(sanatized_input);
                        cout << "The Chosen Decryption from Bombe Reads: " << out_string << endl;
                        outputFile << out_string;
                        output.clear();
                        input.clear();
                        inputFile.close();
	        ...
	        case 5:{   
                ...
                "[1] Set the Rotors" << endl <<
                "[2] Set the Rotor Starting Position?" << endl <<
                "[3] Set the Rotor Kick Values?" << endl <<
                "[4] Set mirror?" << endl <<
                "[2] Set the Switch Board Settings?" << endl <<
                ...
                do {
                    ...
                    switch (settings_option){ //Detects the option
                        case 1:{
                            cout << "Set Rotor 1's Start: " << endl;
                            cin >> rotor1_start;
                        
                            cout << "Set Rotor 2's Start: " << endl;
                            cin >> rotor2_start;
                        
                            cout << "Set Rotor 3's Start: " << endl;
                            cin >> rotor3_start;
                            break;
                        }
                        case 2:{
                            cout << "Please enter 26, non-repeating, capital letters: " << endl;
                            cin >> plug_board_chars;
                            for(int i = 0; i <= 25; i++){
                                plug_board[i] = plug_board_chars[i]
                            }
                            break;
                        }
                        case 3:{
                            cout << "When will Rotor 1 increment?" << endl;
                            cin >> rotor1_kick_value;
                            cout << "When will Rotor 2 increment?" << endl;
                            cin >> rotor2_kick_value;
                            cout << "When will Rotor 3 increment?" << endl;
                            cin >> rotor3_kick_value;
                            break;
                        }
                        default: {
                            cout<<"Invalid Option!";
